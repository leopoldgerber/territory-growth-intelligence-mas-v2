...
from app.models.tables import BudgetStrategyReport, DerivedSignal, DimCountry, OpportunityScore
...
def build_strategy_key(
    ...
    return (
        f'{project_id}:{source.scope}:{source.country_id}:{date_from}:{date_to}:'
        f'{budget_key}:{currency}:{calculation_version}'
    )
